...
const FlatListComponent=({isDark})=>{
    ...
    /*
        ...
        ScrollView은 모든 아이템을 미리 렌더링하고 메모리에 유지하기 때문에 아이템의 수가 많아지면 성능이 저하될 수 있다.
    ...
    */
    ...
    return (
        <FlatList
            ...
            renderItem={({item}) => <PressableComponent isDark={isDark} item={item}/>  // 각 아이템을 렌더링하는 함수로 자식 요소를 컴포넌트로 변환한다. return()문으로도 쓸 수 있지만 보통은 버튼 개체로 구현한다.
            }
        />
    )   
};
